...
- [x] `AreaEvidence` 타입과 `resizeAreaEvidence`, `isAllowedDocument`, `isPdfFile`, `formatFileSize`의 실패 테스트를 작성한다.
- [x] `pnpm test:run -- tests/report-ui.test.ts`에서 새 테스트가 함수 부재로 실패하는지 확인한다.
- [x] 앞쪽 영역 값을 보존하며 1개에서 5개로 크기를 조정하는 최소 구현을 작성한다.
- [x] PDF 전용 검사와 `.hwp`, `.hwpx`, `.pdf` 문서 검사, 사람이 읽기 쉬운 파일 크기 변환을 구현한다.
- [x] 대상 단위 테스트가 통과하는지 확인한다.
...
- [x] 기본 국어 화면에 반영 영역 수 3개와 영역 파일 행 3개가 표시되는 실패 테스트를 작성한다.
- [x] 영역별 PDF가 없으면 생성이 차단되는 실패 테스트를 작성한다.
- [x] 영역명 3개와 PDF 3개, 비밀번호를 입력하면 더미 생성이 실행되는 실패 테스트를 작성한다.
- [x] 평가 계획 파일 1개와 선택 수행평가지 여러 개가 파일명과 크기로 표시되는 실패 테스트를 작성한다.
- [x] 기본 예시문 3종 선택이 textarea에 반영되고 수정 가능한지 검사한다.
- [x] 영역 수와 영역 상태, 파일 상태, 예시 선택 상태를 구현해 테스트를 통과시킨다.
...
- [x] 평가 계획 업로드 카드, 영역별 필수 PDF 행, 선택 수행평가지 카드를 흰색과 파스텔 블루로 구성한다.
- [x] 필수·선택·등록 완료 상태를 글자와 배지로 함께 구분한다.
- [x] 실제 파일 경로 대신 파일명과 크기, 삭제 버튼만 표시한다.
- [x] README에 1단계가 파일 등록 여부만 검증하고 본문 분석은 하지 않는다고 명시한다.
